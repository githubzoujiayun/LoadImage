...
import com.baseapp.loadimage.LoaderImageOptions;
...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        File file = getDiskCacheDir(this, "upload");
        int memorycahesize = (int) Runtime.getRuntime().maxMemory();

        if (!file.exists())
            file.mkdirs();
        LoaderImageOptions options = new LoaderImageOptions(true, file.getAbsolutePath(),
                memorycahesize / 8, 3, 2,10*1024*1024);
        LoadImage.getInstance().init(options);
        GridView gv = (GridView) findViewById(R.id.gv);
        GridViewAdapter adapter = new GridViewAdapter(imageThumbUrls, this);
